...
if(isset($_GET['hapus'])){
    ...
    $book_data = mysqli_fetch_assoc($q);
    
    if($book_data){
        mysqli_query($koneksi, "DELETE FROM buku WHERE id=$id_buku");
        setNotif('Buku "'.$book_data['judul'].'" berhasil dihapus dari katalog!', 'success');
    } else {
...
<head>
    ...
    <title>Data Buku - Perpustakaan Bintang</title>
    <!-- Google Fonts -->
...
<body class="min-h-screen pb-12">
    ...
    <dialog id="modalHapus" class="modal z-50">
        <div class="modal-box p-6 border border-[var(--glass-border)] shadow-2xl relative overflow-hidden">
            ...
            <div class="modal-action flex gap-2 pt-2">
                <button class="btn btn-premium-ghost rounded-xl px-5" onclick="tutupModalHapus()">Batal</button>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
            <div class="text-center sm:text-left">
                <h2 class="text-3xl font-black tracking-tight flex items-center justify-center sm:justify-start gap-3 text-primary-theme">
                    <i data-lucide="database" class="icon-indigo w-8 h-8"></i>
                    <span>Data Buku Master</span>
                </h2>
                <p class="text-[10px] text-secondary-theme opacity-80 mt-1 uppercase tracking-widest font-semibold ml-11">Tampilan Alternatif Katalog Admin</p>
            </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <button onclick="bukaModalHapus(<?= $b['id'] ?>, '<?= htmlspecialchars($b['judul'], ENT_QUOTES) ?>')" class="btn btn-premium-danger btn-xs rounded-xl px-3 py-1 text-xs gap-1 flex items-center justify-center">
    ...
    <script>
        ...
        function bukaModalHapus(id, judul){
            idBuku = id;
            document.getElementById('modalText').innerHTML = 'Apakah Anda yakin ingin menghapus buku <span class="text-[var(--accent-danger)] font-extrabold">"' + judul + '"</span>? Data yang telah dihapus tidak dapat dipulihkan!';
        ...
        document.getElementById('btnYes').onclick = function(){
            window.location = 'buku.php?hapus=' + idBuku;
        }
    </script>
