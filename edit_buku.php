...
if(!$buku){
    setNotif('Buku tidak ditemukan di katalog!', 'error');
...
if(isset($_POST['update'])){
    ...
    $update = mysqli_query($koneksi, "UPDATE buku SET judul='$judul', penulis='$penulis', tahun=$tahun, stok=$stok WHERE id=$id");
    
    if($update){
        setNotif('Sukses! Buku "'.$judul.'" berhasil diupdate.', 'success');
    } else {
        setNotif('Gagal mengupdate data buku!', 'error');
    }
    echo "<script>window.location='kelola_buku.php';</script>";
    exit;
}
...
<head>
    ...
    <title>Edit Buku - Perpustakaan Bintang</title>
    <!-- Google Fonts -->
                ...
                <div class="card-body p-6 sm:p-8 text-secondary-theme">
                    ...
                    <div class="flex flex-col items-center mb-6">
                        <div class="w-12 h-12 rounded-2xl bg-[var(--accent-primary)]/10 border border-[var(--accent-primary)]/20 flex items-center justify-center shadow-lg shadow-[var(--accent-primary)]/10 mb-3 transition-transform hover:scale-110 duration-300">
                            <i data-lucide="edit-3" class="icon-indigo w-6 h-6 filter drop-shadow-[0_0_8px_rgba(177,116,87,0.5)]"></i>
                        </div>
                        <h2 class="text-2xl font-bold text-center tracking-tight text-primary-theme">
                            Edit Informasi Buku
                        </h2>
                        <p class="text-[10px] text-secondary-theme opacity-70 mt-0.5 tracking-wider uppercase font-semibold">Ubah Katalog Perpustakaan</p>
                    </div>
                    ...
                    <form method="POST" class="space-y-4">
                        <div class="form-control w-full">
                            ...
                            <div class="input-icon-wrapper">
                                <input type="text" name="judul" id="inputJudul" value="<?= htmlspecialchars($buku['judul']) ?>" class="input input-glass w-full text-primary-theme" required autocomplete="off">
                        ...
                        <div class="form-control w-full">
                            ...
                            <div class="input-icon-wrapper">
                                <input type="text" name="penulis" id="inputPenulis" value="<?= htmlspecialchars($buku['penulis']) ?>" class="input input-glass w-full text-primary-theme" required autocomplete="off">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="form-control w-full">
                                ...
                                <div class="input-icon-wrapper">
                                    <input type="number" name="tahun" id="inputTahun" value="<?= $buku['tahun'] ?>" min="1900" max="2026" class="input input-glass w-full text-primary-theme" required>
                            ...
                            <div class="form-control w-full">
                                ...
                                <div class="input-icon-wrapper">
                                    <input type="number" name="stok" id="inputStok" value="<?= $buku['stok'] ?>" min="0" class="input input-glass w-full text-primary-theme" required>
                        ...
                        <div class="pt-4">
                            <button type="submit" name="update" class="btn btn-premium w-full text-white font-bold tracking-wide transition-all duration-300 gap-1.5 flex items-center justify-center">
            ...
            <div class="lg:col-span-5 w-full flex flex-col gap-4">
                ...
                <div class="glass-card p-6 flex flex-col justify-between h-[360px] relative overflow-hidden transition-all duration-300 border border-[var(--accent-primary)]/20 shadow-xl" style="background: var(--bg-dark-glow);">
                    ...
                    <div>
                        ...
                        <div class="w-full h-36 rounded-xl bg-gradient-to-br from-[var(--accent-primary)]/15 via-[var(--accent-secondary)]/5 to-[var(--accent-primary)]/5 border border-[var(--glass-border)] flex items-center justify-center mb-5 relative overflow-hidden">
                            ...
                            <div class="absolute bottom-3 right-3">
                                <span class="badge badge-premium-ghost py-1.5 px-3 text-[9px] uppercase tracking-wider shadow-sm" id="previewTahun"><?= $buku['tahun'] ?></span>
                            </div>
                        </div>
                        
                        <!-- Book Title & Author -->
                        <h4 class="font-extrabold text-xl text-primary-theme line-clamp-1" id="previewJudul"><?= htmlspecialchars($buku['judul']) ?></h4>
                        <div class="flex items-center gap-1.5 mt-2.5 text-sm text-secondary-theme opacity-90">
                            ...
                            <span class="font-medium line-clamp-1" id="previewPenulis"><?= htmlspecialchars($buku['penulis']) ?></span>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-[var(--glass-border)] flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-base font-black text-primary-theme mt-0.5" id="previewStok"><?= $buku['stok'] ?> Buku</p>
                        </div>
                        <div>
                ...
                <div class="alert toast-glass shadow-md py-3 px-4 flex items-start gap-3 border border-[var(--glass-border)]">
                    ...
                    <p class="text-[11px] text-secondary-theme leading-relaxed">
                        Perubahan pada form kiri akan langsung tervisualisasi pada kartu pratinjau di atas. Klik "Perbarui Buku" untuk menyimpan ke database.
                    </p>
                </div>
            </div>
